...
import type { KnowledgeFilters, KnowledgeContentType } from "@/features/knowledge/types";
...
interface KnowledgeFilterBarProps {
  ...
  tagOptions: string[];
  onChange: (nextFilters: KnowledgeFilters) => void;
  onReset: () => void;
}

export function KnowledgeFilterBar({ filters, tagOptions, onChange, onReset }: KnowledgeFilterBarProps) {
  ...
  const contentTypes: KnowledgeContentType[] = [
    "concept",
    "formula",
    "problem_pattern",
    "word",
    ...
    <div className="panel flex flex-col gap-4 border-white/40 bg-white/30 p-4 dark:border-stone-700/50 dark:bg-stone-900/40">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          onChange={(e) => onChange({ ...filters, archiveView: e.target.value as KnowledgeFilters["archiveView"] })}
          className="rounded-2xl border border-white/60 bg-white/50 px-3 py-2.5 text-sm font-bold text-stone-700 shadow-sm outline-none transition focus:border-sage-400 dark:border-stone-700/50 dark:bg-stone-800/50 dark:text-stone-300"
        >
          <option value="active">使用中的内容</option>
          <option value="archived">已归档内容</option>
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
        ...
        {/* Content Type Filter */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-black uppercase tracking-widest text-stone-500">类型</span>
          <div className="flex gap-1.5 flex-wrap">
            <FilterChip
              label="全部"
              active={filters.contentType === "all"}
              onClick={() => onChange({ ...filters, contentType: "all" })}
            />
            {contentTypes.map((type) => (
              <FilterChip
                key={type}
                label={knowledgeContentTypeLabelMap[type]}
