...
interface MistakeStatsHudProps {
  totalCount: number;
...
export function MistakeStatsHud({
  totalCount,
  archivedCount,
  importantCount,
  imageCount,
  subjectCounts,
}: MistakeStatsHudProps) {
  return (
    <div className="flex flex-wrap items-center gap-3">
      {/* Overview stats chips */}
      <div className="flex flex-wrap items-center gap-2">
        <StatPill label="有效错题" value={totalCount} accent="text-stone-900 dark:text-white border-stone-300 dark:border-stone-600" />
        {importantCount > 0 && <StatPill label="高优先级" value={importantCount} accent="text-rose-700 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-900/30 border-rose-200 dark:border-rose-800" />}
        {imageCount > 0 && <StatPill label="含图片" value={imageCount} accent="text-sky-700 dark:text-sky-400 bg-sky-50/50 dark:bg-sky-900/30 border-sky-200 dark:border-sky-800" />}
